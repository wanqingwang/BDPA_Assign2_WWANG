package mdpassignment.similarity;
...
public class process_stopwords extends Configured implements Tool {

	private static final Logger LOG = Logger
			.getLogger(process_stopwords.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new process_stopwords(), args);
		System.exit(res);
	}

	public int run(String[] args) throws Exception {

		Job job = Job.getInstance(getConf(), "process_stopwords");
		...
		long counter = job.getCounters().findCounter(CUSTOM_COUNTER.LINES)
				.getValue();
		Path outFile = new Path("LINES.txt");
	...
	public static class Reduce extends
		...
		public void reduce(LongWritable key, Iterable<Text> values,
			...
			reader = new BufferedReader(
					new FileReader(
							new File(
									"/home/cloudera/workspace/Assignment_similarity/output/wordcount.txt")));
			String pattern;
			...
			for (String word : wordsHS) {
				...
				firstprefix = ", ";
				wordswithcountSB.append(word + "_" + wordcount.get(word));
			}
			...
			context.getCounter(CUSTOM_COUNTER.LINES).increment(1);
