...
public class similarity extends Configured implements
		Tool {
	...
	public int run(String[] args) throws Exception {
		System.out.println(Arrays.toString(args));
		Job job = Job.getInstance(getConf(), "similarity");
		...
		long counter = job.getCounters()
				.findCounter(CUSTOM_COUNTER.com_sim).getValue();
		Path outFile = new Path("com_sim.txt");
	...
	public static class Reduce extends Reducer<TextPair, Text, Text, Text> {
		...
		public void reduce(TextPair key, Iterable<Text> values, Context context)
			...
			context.getCounter(CUSTOM_COUNTER.com_sim).increment(1);
