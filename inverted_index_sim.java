...
public class inverted_index_sim extends Configured implements Tool {

	public static enum CUSTOM_COUNTER {
		inverted_index_com_sim,
	};

	public static void main(String[] args) throws Exception {
		System.out.println(Arrays.toString(args));
		int res = ToolRunner.run(new Configuration(),
				new inverted_index_sim(), args);
	...
	public int run(String[] args) throws Exception {
		System.out.println(Arrays.toString(args));
		Job job = Job.getInstance(getConf(), "inverted_index_sim");

		job.setJarByClass(inverted_index_sim.class);
		...
		long counter = job.getCounters()
				.findCounter(CUSTOM_COUNTER.inverted_index_com_sim).getValue();
		Path outFile = new Path("inverted_index_com_sim.txt");
	...
	public static class Reduce extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			reader = new BufferedReader(
					new FileReader(
							new File(
									"/home/cloudera/workspace/Assignment_similarity/input/tokensinline2.txt")));
			String pattern;
			...
			if (wordsL.size() > 1) {
				...
				for (String pair : pairs) {
					...
					context.getCounter(CUSTOM_COUNTER.inverted_index_com_sim)
							.increment(1);
